...
export default function KycCountdownOverlay() {
        ...
        const refreshSession = async () => {
            try {
                const res = await axios.post(
                    `${import.meta.env.VITE_API_BASE_URL}/api/kyc/sessions/new`,
                    {
                        kycId: kycData._id,
    ...
    useEffect(() => {
        ...
        const updateCountdown = () => {
            ...
            if (remaining === 0) {
                ...
                axios
                    .post(
                        `${import.meta.env.VITE_API_BASE_URL}/api/kyc/expire/${kycId}`,
                        {
                            kycId: kycData._id,
                        },
                        {
    ...
    const handleConfirmClick = async () => {
        ...
        try {
            ...
            // }
            if (response.data.success) {
                localStorage.setItem("kycConfirmed", "true"); // flag cho Upload biết là được phép
                navigate(`/upload/${kycId}`, { replace: true }); // chuyển trang và chặn quay lại
              }
    ...
    return (
        <div className="relative">
            ...
            <div className="fixed top-2 left-2 bg-white/80 p-2 rounded shadow text-xs z-50">
                ...
                <div>
                    Ví: {kycData.wallet.slice(0, 6)}...{kycData.wallet.slice(-4)}
                </div>
                ...
                {confirmed && <div className="mt-1 text-green-600 font-semibold">✅ Đã gửi xác nhận!</div>}
            </div>
